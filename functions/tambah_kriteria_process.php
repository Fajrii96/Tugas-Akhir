...
if(isset($_POST['submit'])){
    ...
    if(has_dupes($array_to_check_dup) == false){
        ...
        if($rowcount > 0){
            ...
            $message = "Gagal, batas kriteria yang dapat disimpan telah mencapai batas maksimal (10). Silahkan hapus kriteria terlebih dahulu.";
            echo "<script>alert('$message'); window.location.replace('../pages/tambah_kriteria.php');</script>";
        }
        else{
            ...
            if($result){ 
                ...
                if($result){
                    ...
                    $del = mysqli_query($conn,"DELETE FROM daftar_kriteria_static where kriteria = '$nama_kriteria'");
                    $nk_lowered = strtolower($nama_kriteria);
                    $del = mysqli_query($conn, "ALTER TABLE tempat_wisata_tb DROP COLUMN {$nk_lowered};");
                    $message = "GAGAL - Terdapat masalah pada pembuatan tabel kriteria baru.";
                    echo "<script>alert('$message'); window.location.replace('../pages/admin_page.php');</script>";
                }
